...
public class _03_MurderOfCrows {
	...
	private void findTheDiamond() {
		for (Crow crow : theMurder) {
			for (String food : crow.getStomachContents()) {
				if (food.equals("diamond")) {
					thief = true;
				}
			}
			if (thief) {
				String t = crow.getName();
				System.out.println(t + " is the thief!");
				if (count == 0) {
					System.out.println("No crows were killed before he was found :)");
				} else if (count == 1) {
					System.out.println(count + " crow was killed before he was found :(");
				} else {
					System.out.println(count + " crows were killed before he was found :(");
				}
